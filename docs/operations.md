...
`signal-sim-market-scope` is centered on simulations work in C#. The technical target is: Create a C# reference implementation for market workflows, centered on storage recovery, log and snapshot fixtures, and replay consistency checks.
...
The score is deterministic. The current threshold is 161. Risk has a penalty of 5, latency has a penalty of 3, and weight contributes a bonus of 6.
